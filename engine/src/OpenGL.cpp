...
namespace engine::graphics {
    ...
    unsigned int OpenGL::generate_framebuffer() {
        unsigned int FBO;
        CHECKED_GL_CALL(glGenFramebuffers, 1, &FBO);
        return FBO;
    }

    std::pair<unsigned int, unsigned int> OpenGL::generate_two_framebuffers() {
    ...
    void OpenGL::bind_framebuffer(unsigned int FBO) {
        CHECKED_GL_CALL(glBindFramebuffer, GL_FRAMEBUFFER, FBO);
    }

    std::pair<unsigned int, unsigned int> OpenGL::generate_two_framebuffer_textures() {
        unsigned int colorBuffers[2];
        CHECKED_GL_CALL(glGenTextures, 2, colorBuffers);
        return {colorBuffers[0], colorBuffers[1]};
    }

    unsigned int OpenGL::generate_framebuffer_texture() {
        unsigned int texture;
        CHECKED_GL_CALL(glGenTextures, 1, &texture);
        return texture;
    }

    void OpenGL::set_up_framebuffer_texture(unsigned int window_width, unsigned int window_height, unsigned int texture, int attachment_pos) {
    ...
    void OpenGL::bind_texture(unsigned int texture) {
        CHECKED_GL_CALL(glBindTexture, GL_TEXTURE_2D, texture);
    }

    void OpenGL::generate_depth_buffer(unsigned int window_width, unsigned int window_height) {
    ...
    void OpenGL::set_up_attachments() {
        unsigned int attachments[2] = {GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1};
        CHECKED_GL_CALL(glDrawBuffers, 2, attachments);
    }

    void OpenGL::activate_texture(unsigned int texture, int pos) {
        CHECKED_GL_CALL(glActiveTexture, GL_TEXTURE0 + pos);
        CHECKED_GL_CALL(glBindTexture, GL_TEXTURE_2D, texture);
    }

    unsigned int OpenGL::set_up_quad() {
    ...
    void OpenGL::render_quad(unsigned int VAO) {
    ...
    void OpenGL::delete_framebuffer(unsigned int FBO) {
        CHECKED_GL_CALL(glDeleteFramebuffers, 1, &FBO);
    }

    void OpenGL::delete_vertex_array(unsigned int VAO) {
