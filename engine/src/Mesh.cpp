...
void Mesh::setup_instance_matrices(const std::vector<glm::mat4> &instanceMatrices) {
    if (m_instance_vbo == 0)
...
void Mesh::draw_instanced(const Shader *shader, int instanceCount) {
    std::unordered_map<std::string_view, uint32_t> counts;
    std::string uniform_name;
    uniform_name.reserve(32);
    for (int i = 0; i < m_textures.size(); i++) {
    ...
    glBindVertexArray(m_vao);
    glDrawElementsInstanced(GL_TRIANGLES, m_num_indices, GL_UNSIGNED_INT, 0, instanceCount);
    glBindVertexArray(0);
}
